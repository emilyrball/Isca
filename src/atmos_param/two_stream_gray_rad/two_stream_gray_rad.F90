...
logical                             :: do_read_dust=.true.
type(interpolate_type),save         :: dust_interp
character(len=256)                  :: dust_file='dust_file'
character(len=256)                  :: dust_variable_name='dust'
...
namelist/two_stream_gray_rad_nml/ solar_constant, del_sol, &
           ...
           do_read_dust, dust_file, dust_variable_name, &
...
else if(uppercase(trim(rad_scheme)) == 'DUST') then
  lw_scheme = B_DUST
  call error_mesg('two_stream_gray_rad','Using DUST radiation scheme.', NOTE)
else
...
case(B_FRIERSON, B_BYRNE, B_DUST)
...
if(do_read_dust)call interpolator_end(dust_interp)
